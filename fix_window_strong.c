// Build individually without make:
// mpicc -O3 -march=native -fopenmp -std=c11 -Wall -Wextra \
//   fix_window_strong.c forest_model.c load_csv.c load_bin_mpi.c \
//   -o train_pt_mpi -lm
// Run:
//   Classic (per-rank iters):  mpirun -np P ./train_pt_mpi --csv covtype.csv [--iters K] ...
//   Strong scaling (fixed global work):
//        mpirun -np P ./train_pt_mpi --csv covtype.csv --K_total Kglobal [--W 3] ...
//   cmd line flags :
//        --iters K: per-rank iterations
//        --K_total Kglobal: total global chain steps (strong scaling); program uses K_eff = ceil(Kglobal/P)
//        --W W: override swap window (we pin W across P for fair scaling); else computed from P,S
//        --S 0.4 --eta_lo 3e-3 --eta_hi 2e-2 --gamma0 0.05 --gdecay 1e-4 --val 0.2 --snap_stride 1000 --seed 42
//automate investigation strong scaling of this with run_strong_scaling.sh
#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
//Simple CLI helpers => feed in parameters like gamma (adaptation rate), eta_lo, eta_hi (lowest and highest learning rates), S = target swap rate between adjacent chains, K = total iters, W= window size we use to calculate S
...
static inline int window_size_opt(int P, double S){
  ...
  // W = ceil((log P + log log P)/(-log(1-S)))  [DEO★]
...
int main(int argc, char** argv){
  ...
  MPI_Comm_set_errhandler(comm, MPI_ERRORS_RETURN);
    // iterations (legacy per-rank) vs strong-scaling global budget
  const int K_user  = arg_int (argc,argv,"--iters", 20000); // per-rank iterations (legacy version of driver)
const int trace_stride = arg_int(argc, argv, "--trace_stride", 0); // 0 = off => for tracking changes to C and A over time per rank
    if (trace_stride > 0) setvbuf(stdout, NULL, _IOLBF, 0);
    const int stab_stride = arg_int(argc, argv, "--stab_stride", 0); // 0=off
  const int K_total = arg_int (argc,argv,"--K_total", 0); // global chain steps (strong scaling) -> 0=off
    const char* csv = arg_str(argc,argv,"--csv",NULL);
    const char* bin = arg_str(argc,argv,"--bin",NULL);
    if(!csv && !bin){
      if(rank==0) fprintf(stderr,"pass --csv <covtype.csv> or --bin <covtype.bin>\n");
      MPI_Abort(comm,1);
    }
  ...
  const int W_cli = arg_int(argc,argv,"--W", -1); // optional: pin W instead of auto formula
  const int P = size; // 1 replica (chain) per MPI process
    const int W = (W_cli >= 1) ? W_cli : window_size_opt(P, S);
    // effective per-rank iterations: use global K_total if provided (strong scaling), else legacy K_user
      const int K_eff = (K_total > 0) ? ((K_total + P - 1) / P) : K_user;
    if (rank==0 && K_total > 0 && W_cli < 1) {
      fprintf(stderr, "[warn] strong scaling with auto W=%d (depends on P). "
              "For apples-to-apples comm, pass --W <const> (e.g., --W %d)\n", W, W);
      fflush(stderr);
    }
    
      if(rank==0){
        if (K_total > 0) {
          printf("[P=%d] W=%d  S=%.3f  K_eff=%d (from K_total=%d)  B=%d\n", P, W, S, K_eff, K_total, BATCH_SIZE);
        } else {
          printf("[P=%d] W=%d  S=%.3f  K=%d  B=%d\n", P, W, S, K_user, BATCH_SIZE);
        }
      } //rank 0 will start off run by printing the chosen parameters
    if (rank==0 && K_total > 0) {
      int K_global_eff = P * K_eff; // actual global steps executed (ranks times local steps)
      int overshoot = K_global_eff - K_total; //difference
      printf("[strong] requested K_total=%d  actual_global=%d  overshoot=%d\n", K_total, K_global_eff, overshoot);
      fflush(stdout);
    }
    ...
    if (bin) {
      ...
      if (load_bin_mpi_allrep(bin, &X, &y, &Nall, comm) != 0) {
        ...
        t_load = MPI_Wtime() - t0;
          t_bcast = 0.0; // no broadcast in MPI-IO all-replica path
      if (rank==0) {
          printf("[io] load=%.3fs  bcast=%.3fs  N=%d  dim=%d\n", t_load, t_bcast, Nall, INPUT_DIM);
        printf("[io] MPI-IO load OK: %s  N=%d dim=%d out=%d\n",
               bin, Nall, INPUT_DIM, OUTPUT_DIM);
        fflush(stdout);
      }
    } else {
      // CSV load with Bcast fallback
      ...
      if(rank==0){
        ...
        for (int i=0;i<Nall;++i) { int yv=yall[i]; if (0<=yv && yv<OUTPUT_DIM) cnt[yv]++; }
      ...
      MPI_Bcast(y, Nall, MPI_INT32_T, 0, comm);
    ...
    MPI_Barrier(comm);
  //train/test split with a shared index permutation
    int *perm_split = (int*)malloc((size_t)Nall*sizeof(int));
    if(rank==0){
        //random permutation of row indices with a Fisher-Yates shuffle, seed is fixed so we can reproduce results across runs
      srand(seed);
  ...
  const int Ntrain = Nall - Ntest; //the first Ntrain indices that were permuted before will go to the training set
  ...
  const int epochs = (K_eff + iters_per_epoch - 1) / iters_per_epoch; //ceiling so inner loop can run at least K_eff update steps
  ...
  double t_compute=0, t_energy_comm=0, t_swap_comm=0, t_allreduce=0;
    ...
    // keep a previous model snapshot to measure step norm / growth
  ...
  Model* snaps = NULL; int nsnaps=0;
    if(rank==0 && snap_stride>0) snaps = (Model*)malloc(((K_eff/snap_stride)+2)*sizeof(Model));
  ...
  for(int e=0; e<epochs; ++e){
    ...
    if (++global_iter > K_eff) break;
      ...
      t_compute += MPI_Wtime() - t0;
        ...
        if (stab_stride > 0 && (k % stab_stride) == 0) {
          ...
          // gradient norm: already computed by forward_backward_batch()
          double gnorm = grad_l2_norm();

          // precision gap on the already-built swap batch (shared across ranks)
          double L32 = myE; // batch_loss_only() you computed just below the update
          ...
          // aggregate (mean/max) across ranks
          ...
          // refresh snapshot for next stride
          prev_model = model;
      ...
      // correction buffer update (Allreduce on a bunch of ints)
        ...
        if (trace_stride > 0 && (k % trace_stride) == 0) {
          ...
          printf("[trace] r=%d k=%d avgA=%.6f C=%.6f d_compute=%.6f d_ecomm=%.6f d_swap=%.6f d_allred=%.6f cum_compute=%.6f cum_ecomm=%.6f cum_swap=%.6f cum_allred=%.6f\n",
                 rank, k, avgA, C, dcomp, de, dsw, dar, t_compute, t_energy_comm, t_swap_comm, t_allreduce);
          fflush(stdout);
    ...
    // Reduce per-rank timers to system summaries
