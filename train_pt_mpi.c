// Optionally build by itself as :
/* mpicc -O3 -march=native -fopenmp -std=c11 -Wall -Wextra \
//   train_pt_mpi.c forest_model.c load_csv.c load_bin_mpi.c \
//   -o train_pt_mpi -lm
// Run:   mpirun -np P ./train_pt_mpi --csv covtype.csv [--iters K] [--S 0.4] [--eta_lo 3e-3] [--eta_hi 6e-1]
       [--gamma0 0.05] [--gdecay 1e-4] [--val 0.2] [--snap_stride 1000] [--seed 42]*/
#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static inline int window_size_opt(int P, double S){
  ...
  // W = ceil((log P + log log P)/(-log(1-S)))  [DEO swap scheme]
...
int main(int argc, char** argv){
  ...
  Model model; init_model(&model); //give random weights and biases to initialise
    ...
    if (P == 1) {
      eta[0] = eta_lo; // <- single chain: just use eta_lo cold learning rate
    } else {
      for (int p=0; p<P; ++p) {
  ...
  double t_compute=0, t_energy_comm=0, t_swap_comm=0, t_allreduce=0;
    // Stability tracking (previous model snapshot)
    int have_prev = 0;
    Model prev_model;
    ...
    for(int b0=0; b0<Ntrain; b0+=BATCH_SIZE){
      ...
      t_compute += MPI_Wtime() - t0;
        ...
        if (stab_stride > 0 && (k % stab_stride) == 0) {
          // Parameter norm and step size in parameter space
          ...
          // Gradient norm from last forward_backward_batch()
          double gnorm = grad_l2_norm();

          // FP32–FP64 loss gap on the same swap batch
          double L32 = myE;
          ...
          v = gnorm; MPI_Reduce(&v,&agg_gn.sum,   1,MPI_DOUBLE,MPI_SUM,0,comm); MPI_Reduce(&v,&agg_gn.max, 1,MPI_DOUBLE,MPI_MAX,0,comm);
          v = rel_gap; MPI_Reduce(&v,&agg_gap.sum,  1,MPI_DOUBLE,MPI_SUM,0,comm); MPI_Reduce(&v,&agg_gap.max,  1,MPI_DOUBLE,MPI_MAX,0,comm);

          if (rank==0) {
            double Pd = (double)size; // same as P
  ...
  if(rank==0){
    ...
    if (nsnaps>0){
      // simple ensemble over snapshots for averaging
      double correct = 0.0;
